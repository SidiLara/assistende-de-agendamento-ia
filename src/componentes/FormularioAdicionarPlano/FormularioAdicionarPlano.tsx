...
export const FormularioAdicionarPlano = ({ onSalvar, onCancelar }: FormularioAdicionarPlanoProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!nome.trim() || isNaN(valorNumerico) || valorNumerico <= 0) {
            setErro('Nome e valor (válido e positivo) são obrigatórios.');
            return;
        }
        setErro('');
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label htmlFor="nome" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nome do Plano</label>
                <input
                    type="text"
                    id="nome"
                    value={nome}
