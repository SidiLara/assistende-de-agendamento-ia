...
import { DashboardDeAcoes } from '../../componentes/DashboardDeAcoes';
...
export const MenuCrm: React.FC = () => {
    ...
    const acoesDisponiveis = [
        { titulo: "Gerenciar Clientes", linkPara: "/crm/clientes", icone: <UsersIcon /> },
        { titulo: "Gerenciar Planos", linkPara: "/crm/planos", icone: <ClipboardListIcon /> },
        { titulo: "Financeiro", linkPara: "/crm/financeiro", icone: <CurrencyDollarIcon /> },
        { titulo: "Ver Estatísticas", linkPara: "/crm/estatisticas", icone: <ChartBarIcon /> },
        { titulo: "Ver Auditoria", linkPara: "/crm/auditoria", icone: <DocumentReportIcon /> },
        ...(user?.role === 'Admin' ? [{ titulo: "Gerenciar Usuários", linkPara: "/crm/usuarios", icone: <ShieldCheckIcon /> }] : []),
    ];

    return (
        <div>
            <h1 className="text-3xl font-bold mb-2">Bem-vindo(a) ao CRM, {user?.email}!</h1>
            <p className="text-gray-500 dark:text-gray-400 mb-8">Selecione uma das opções abaixo para começar.</p>
            <DashboardDeAcoes acoes={acoesDisponiveis} />
        </div>
    );
};