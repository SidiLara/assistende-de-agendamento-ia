...
import { FormularioAdicionarConsultorProps } from './FormularioAdicionarConsultor.props';
...
export const FormularioAdicionarConsultor = ({ consultorParaEditar, planosDisponiveis, onSalvar, onCancelar }: FormularioAdicionarConsultorProps) => {
    ...
    React.useEffect(() => {
        if (consultorParaEditar) {
            setNome(consultorParaEditar.nome);
            setPlano(consultorParaEditar.plano);
            setTelefone(consultorParaEditar.telefone);
            setDataInicio(consultorParaEditar.dataInicio || new Date().toISOString().split('T')[0]);
            setTipoPagamento(consultorParaEditar.tipoPagamento || 'Fixo');
            setNumeroParcelas(consultorParaEditar.numeroParcelas || 12);
        } else {
            setNome('');
            setPlano('Básico');
            setTelefone('');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setErro('');
        onSalvar({ nome, plano, telefone, dataInicio, tipoPagamento, numeroParcelas: tipoPagamento === 'Parcelado' ? numeroParcelas : undefined });
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="nome" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nome do Consultor</label>
                    <input type="text" id="nome" value={nome} onChange={(e) => setNome(e.target.value)} className="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm bg-white dark:bg-dark-tertiary" placeholder="Ex: João da Silva"/>
                </div>
                <div>
